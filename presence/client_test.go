...
func TestKeepAliveUsesServerHeartbeat(t *testing.T) {
	...
	started := time.Now()
	secondUpdateAfter := make(chan time.Duration, 1)
	client := New(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		if posts.Add(1) == 1 {
			...
			secondUpdateAfter <- time.Since(started)
		}
		return &http.Response{
			StatusCode: http.StatusOK,
			Status:     "200 OK",
	...
	if elapsed := <-secondUpdateAfter; elapsed < 900*time.Millisecond {
		t.Fatalf("second update after %s, want at least 900ms", elapsed)
	}
	if err := client.CloseContext(context.Background()); err != nil {
		t.Fatalf("CloseContext returned error: %v", err)
	}
...
func TestKeepAliveDoesNotUpdateCanceledContext(t *testing.T) {
	var posts atomic.Int32
	client := New(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		posts.Add(1)
		return &http.Response{StatusCode: http.StatusOK, Body: http.NoBody, Request: req}, nil
	...
	if err := client.KeepAlive(ctx, TitleRequest{}); err != context.Canceled {
...
func TestCloseContextStopsKeepAliveBeforeRemovingPresence(t *testing.T) {
	updated := make(chan struct{})
	var posts, deletes atomic.Int32
	client := New(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		switch req.Method {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- client.KeepAlive(context.Background(), TitleRequest{State: StateActive})
	}()
	<-updated
	if err := client.CloseContext(context.Background()); err != nil {
		t.Fatalf("CloseContext returned error: %v", err)
	...
	if err := <-errCh; err != nil {
		t.Fatalf("KeepAlive returned error after close: %v", err)
	}
	if got := posts.Load(); got != 1 {
		t.Fatalf("POST requests = %d, want 1", got)
	}
